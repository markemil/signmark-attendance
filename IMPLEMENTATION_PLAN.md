...
**Note on camera testing:** the sandboxed dev browser this session uses blocks `getUserMedia` entirely, so the live capture path (Epic 3.1) is verified structurally (viewfinder/preview/retake states, permission-denied handling) but not against real camera hardware. The clock-in/out _logic_ it feeds into — server timestamps, shift/overnight math, hours, double-punch guards — is verified directly against the backend. Testing the actual capture-to-photo flow on a real phone is still worth doing before this ships.

**Note on mobile responsiveness (2026-08-21):** the employee flow was mobile-first from the start, but the admin flow (Dashboard, Calendar, Day Detail, Holidays, Add Employee, and the manual-punch/export modals) was built desktop-first and had never actually been checked at 390px — DESIGN.md's own requirement that the admin flow "never horizontally scroll at 390px" was unverified. Fixed: added `.admin-content`/`.header-row`/`.field-grid-2`/`.scroll-x` responsive utility classes to `components.css` (inline styles can't respond to media queries, so layout-critical properties were moved into these classes), wrapped the calendar's 7-column month grid in its own horizontal-scroll container per DESIGN.md §4 instead of squishing to unreadable cell widths, collapsed all 2-column forms to one column below 640px, and fixed a genuine table-layout bug on the Holidays page where a fixed-width action column could force the row wider than its container. Verified with a script-based overflow check (`document.documentElement.scrollWidth` vs `window.innerWidth`) plus visual screenshots at both 390px and 768px on every admin page — zero page-level horizontal overflow anywhere, confirmed rather than assumed.
